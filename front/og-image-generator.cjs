...
// --- 클럽용 OG 이미지 생성 ---
async function generateClubOgImage(club) {
  const cacheKey = `club_${club.id}_${club.memberCount}`;
  const cached = getCached(cacheKey);
  if (cached) return cached;

  const title = club.name || '테니스 클럽';
  const subtitle = club.region || '';
  const memberInfo = `${club.memberCount || 0}명 활동 중`;
  const desc = club.description
    ? truncate(club.description, 30)
    : '';
  const description = desc ? `${memberInfo} | ${desc}` : memberInfo;
...
module.exports = { generateScheduleOgImage, generateClubOgImage, initBackground };
