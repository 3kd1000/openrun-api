...
import { ArrowLeftIcon, PlusIcon, ChevronDownIcon, ChevronUpIcon } from '../../components/common/Icons';
...
const ClubExplorePage: React.FC = () => {
  ...
  const [interclubRecruit, setInterclubRecruit] = useState<PublicRecruitSchedule[]>([]);
  ...
  const EXPAND_KEY = "openrun_explore_sections_v1";
  ...
  useEffect(() => {
    ...
    try {
      ...
      const parsed = raw ? (JSON.parse(raw) as Partial<{ guestExpanded: boolean; interclubExpanded: boolean; membersExpanded: boolean }>) : null;
      if (parsed && typeof parsed.guestExpanded === "boolean") setGuestExpanded(parsed.guestExpanded);
      if (parsed && typeof parsed.interclubExpanded === "boolean") setInterclubExpanded(parsed.interclubExpanded);
      if (parsed && typeof parsed.membersExpanded === "boolean") setMembersExpanded(parsed.membersExpanded);
    } catch {
      // ignore
    }
  ...
  const persistExpand = (next: Partial<{ guestExpanded: boolean; interclubExpanded: boolean; membersExpanded: boolean }>) => {
    try {
      const raw = localStorage.getItem(EXPAND_KEY);
      const prev = raw ? (JSON.parse(raw) as Record<string, unknown>) : {};
      const merged = { ...prev, ...next };
      localStorage.setItem(EXPAND_KEY, JSON.stringify(merged));
    } catch {
      // ignore
    }
  };
  ...
  const fetchRecruit = async (type: RecruitType) => {
    try {
      const response = await axiosInstance.get<PublicRecruitSchedule[]>('/schedules/recruit', {
        params: { type, limit: 5 },
      });
      ...
      <div className="club-explore-page__recruit-sections">
        <div className="club-explore-page__recruit-section">
          <div className="club-explore-page__recruit-header">
            <button className="club-explore-page__recruit-title-btn" type="button" onClick={toggleGuest}>
              {guestExpanded ? <ChevronUpIcon size={18} /> : <ChevronDownIcon size={18} />}
              <span className="club-explore-page__recruit-title">게스트 모집 중</span>
            </button>
            <button
          ...
          {!guestExpanded ? null : guestRecruit.length === 0 ? (
            <div className="club-explore-page__recruit-empty">모집 중인 일정이 없습니다.</div>
          ) : (
            <div className="club-explore-page__recruit-list">
              {guestRecruit.map((x) => (
                <button
                  ...
                  onClick={() => navigate(`/clubs/${x.clubId}/guest-recruit/${x.scheduleId}`)}
                >
                  <div className="club-explore-page__recruit-item-title">{x.clubName}</div>
        ...
        <div className="club-explore-page__recruit-section">
          <div className="club-explore-page__recruit-header">
            <button className="club-explore-page__recruit-title-btn" type="button" onClick={toggleInterclub}>
              {interclubExpanded ? <ChevronUpIcon size={18} /> : <ChevronDownIcon size={18} />}
              <span className="club-explore-page__recruit-title">교류전 모집 중</span>
            </button>
            <button
              className="club-explore-page__recruit-more"
            ...
            <div className="club-explore-page__recruit-list">
              {interclubRecruit.map((x) => (
                <button
                  key={`I:${x.scheduleId}`}
                  className="club-explore-page__recruit-item"
                  onClick={() => navigate(`/clubs/${x.clubId}/interclub-recruit/${x.scheduleId}`)}
                >
                  <div className="club-explore-page__recruit-item-title">{x.clubName}</div>
                  <div className="club-explore-page__recruit-item-meta">
                    {new Date(x.scheduledAt).toLocaleString()} · {x.courtName}
        ...
        <div className="club-explore-page__recruit-section">
          <div className="club-explore-page__recruit-header">
            <button className="club-explore-page__recruit-title-btn" type="button" onClick={toggleMembers}>
              {membersExpanded ? <ChevronUpIcon size={18} /> : <ChevronDownIcon size={18} />}
              <span className="club-explore-page__recruit-title">신규회원 모집 중</span>
            </button>
            <button
              className="club-explore-page__recruit-more"
          ...
          {!membersExpanded ? null : recruitClubs.length === 0 ? (
            <div className="club-explore-page__recruit-empty">모집 중인 클럽이 없습니다.</div>
          ) : (
            <div className="club-explore-page__recruit-list">
              {recruitClubs.map((c) => (
                <button
                ...
                >
                  <div className="club-explore-page__recruit-item-title">{c.name}</div>
                  <div className="club-explore-page__recruit-item-meta">{c.region ?? "-"}</div>
                  {isNotEmpty(c.description) ? (
                    <div className="club-explore-page__recruit-item-meta">{c.description}</div>
                  ) : null}
                </button>
              ))}
            </div>
