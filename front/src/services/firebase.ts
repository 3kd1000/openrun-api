...
export const setLoginExpiry = (isNewLogin: boolean = true) => {
    if (isNewLogin) {
        ...
        console.log(`✅ 로그인 만료 시간 설정: ${expiryDate.toLocaleString()} (${AUTO_LOGIN_DAYS}일 후)`);
    }
};
...
export const isLoginExpired = (): boolean => {
    ...
    if (now > expiryDate) {
        console.warn(`⏰ 로그인 세션 만료: ${expiryDate.toLocaleString()} < 현재 ${now.toLocaleString()}`);
        return true;
    }
...
export const signInWithGooglePopup = async (): Promise<{ user: User; idToken: string } | null> => {
    try {
...
export const getCurrentToken = async (): Promise<string | null> => {
    // 1. 로그인 세션 만료 체크
    if (isLoginExpired()) {
        console.warn(`⏰ 로그인 세션 만료됨 (${AUTO_LOGIN_DAYS}일 경과) → 자동 로그아웃`);
        await handleSessionExpiry();
        return null;
    }
